...
void dxRenderDeviceRender::overdrawEnd()
{
	...
	for (int I = 0; I < 12; I++)
	{
		...
		FVF::TL pv[4];
		pv[0].set(float(0), float(RCache.get_render_height()), c, 0, 0);
		pv[1].set(float(0), float(0), c, 0, 0);
		pv[2].set(float(RCache.get_render_width()), float(RCache.get_render_height()), c, 0, 0);
		pv[3].set(float(RCache.get_render_width()), float(0), c, 0, 0);
